...
/* save space width of monospace font */
static int global_space_width;
...
/* Check if the font contains CJK Ideograph 'number one', 0x4E00,
   It should be ok for Chinese/Japanese font.
   or contains Korean script syllable 'Ka',0xAC00,
   because korean fonts may not have any Chinese characters at all.
   take from xterm.*/
static int
is_cjk_font(Display * dpy, XftFont * font)
{
    if(XftCharExists(dpy, font, 0x4E00) ||
	XftCharExists(dpy, font, 0xAC00))
	return 1;
    return 0;
}

static int get_font_width_differ(struct xftfont_info *xftfont_info)
{
    int differ = 0;
    XftFont *xftfont = NULL;
    if(xftfont_info) {
	xftfont = xftfont_info->xftfont;
	if(is_cjk_font(xftfont_info->display,xftfont)) {
	double pixelsize = 0;
	FcResult result = FcPatternGetDouble(xftfont->pattern, FC_PIXEL_SIZE ,0,&pixelsize);
	if(result == FcResultMatch)
	    differ = global_space_width * 2 - (int)pixelsize;
	/* pixelsize of some pcf/bdf fonts is bigger than 
	   2*space_width of monospace fonts,
...
xftfont_open (FRAME_PTR f, Lisp_Object entity, int pixel_size)
{
  ...
  if (spacing != FC_PROPORTIONAL
    ...
    {
      font->min_width = font->average_width = font->space_width = global_space_width
...
xftfont_text_extents (struct font *font, unsigned int *code, int nglyphs, struct font_metrics *metrics)
{
  struct xftfont_info *xftfont_info = (struct xftfont_info *) font;
  XGlyphInfo extents;
  int differ, left_differ, right_differ;

  block_input ();
  XftGlyphExtents (xftfont_info->display, xftfont_info->xftfont, code, nglyphs,
		   &extents);
  differ = get_font_width_differ(xftfont_info);
  left_differ = differ>>1; //get half, maybe not divided exactly 
  right_differ = differ - left_differ;
  unblock_input ();
  if (metrics)
    {
      metrics->lbearing = - extents.x + left_differ;
      metrics->rbearing = - extents.x + extents.width + right_differ;
      metrics->width = extents.xOff + differ;
      metrics->ascent = extents.y;
      metrics->descent = extents.height - extents.y;
    }
  return extents.xOff + differ;
}
              ...
              bool with_background)
{
  ...
  if (s->padding_p)
    for (i = 0; i < len; i++)
     ...
     int multi = 2;
     int differ = get_font_width_differ(xftfont_info);
     int left_differ = differ>>1;
     /* if differ is zero, just keep unchanged*/
     if(!differ)
       XftDrawGlyphs (xft_draw, &fg, xftfont_info->xftfont, x, y, code, len);
     else
       //draw glyphs one by one and adjust the offset.
       for (i = 0; i < len; i++) {
	  int offset = global_space_width * i;
	  offset = offset * multi + left_differ;
	  XftDrawGlyphs (xft_draw, &fg, xftfont_info->xftfont,
	     x+offset, y, code+i, 1);
       }
  }
