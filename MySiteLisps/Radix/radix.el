...
;; Copyright (C) 1994, 1995, 1996 Simon Marshall.
...
;; LCD Archive Entry:
;; radix|Simon Marshall|simon@gnu.ai.mit.edu|
;; Functions for the manipulation of integers in any radix (or base).|
;; 16-May-1996|1.06|~/misc/radix.el.Z|
...
;;
;; 1.00--1.01:
;; - Emacs 18: Define missing functions etc. (Francesco Potorti`)
;; 1.01--1.02:
...
;; - Emacs 18: Added `string-to-number'
...
;; - Corrected Copyleft
;; 1.05--1.06:
;; - Corrected dumb spelling mistake in `digit-to-value' error message
;; - Made `number-to-decimal' return a string if `number' is negative
;; - Made `decimal-to-number' return "0" if `number' is zero
;; - Made `interactive' functions succeed `autoload' cookies
;; - Made `apply-operator' use `completing-read' for operator
...
Deals with integers only."
  (let ((number (if (stringp number) number (number-to-string number))))
    (if (eq (aref number 0) ?-)
        (concat "-" (number-to-decimal (substring number 1) radix))
...
NUMBER may be a number or string.  Does no range checking of NUMBER.
Deals with integers only."
  (let ((number (if (numberp number) number (string-to-number number))))
    (cond ((= number 0) "0")
          ((< number 0) (concat "-" (decimal-to-number (abs number) radix)))
          (t
...
See `number-to-decimal' and `decimal-to-number'."
  (interactive
   (let* ((op (intern (completing-read "Apply: " obarray 'fboundp t)))
  ...
  (let* ((convert (function (lambda (n)
                              (string-to-number (number-to-decimal (car n) (cdr n))))))
         (val (apply operator (mapcar convert numbers)))
...
lower case.  Does range checking of DIGIT."
  (let* ((ascii (if (stringp digit) (string-to-char digit) digit))
    ...
    (if (< value radix)
        value
      (error "Digit `%c' too big for radix %d" ascii radix))))

(defun value-to-digit (value)
  "Return string digit representing decimal VALUE.
Alphabetic digits are lower case.  Does no range checking of VALUE."
